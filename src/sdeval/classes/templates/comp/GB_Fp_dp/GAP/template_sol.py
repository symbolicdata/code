"""
This is the template for extracting the solution for the computation
problem of computing a Groebner basis of an ideal in a commutative
polynomial ring over a finite field from the output of the computer
algebra system GAP.
...
def extractSolution(outpString):
    ...
    computation performed in GAP, using the executable code that was
    ...
    :param outpString: The String that was returned by the GAP-execution
    :type  outpString: str
    ...
    solEndStrPos = solStr.index("]")
    solStr = solStr[solBeginPos:solEndStrPos]
    if (solStr.strip()==""):
    ...
    #Adding characteristic, if positive
    if (characteristic):
        tempNodeCharacteristic =\
            tempNode.appendChild(result.createElement("characteristic"))
        tempNodeCharacteristic.appendChild(result.createTextNode(str(characteristic)))
